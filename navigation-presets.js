...
function getVisibleNavIds() {
    return game.scenes.filter((s) => s.navigation).map((s) => s._id);
}
...
function alphaSortPresets(presets) {
    let sorted = Object.values(presets).sort(function (a, b) {
        ...
        else if (a.titleText > b.titleText) return 1;
        return 0;
    });
    return sorted;
}
    ...
    static getPresets() {
        ...
        if (game.user.isGM) {
            ...
            return Object.keys(allPresets)
                .filter((x) =>
                    allPresets[x].sceneList.some((y) => game.scenes.get(y)?.ownership.default != 0 || game.scenes.get(y)?.active)
                )
                .reduce((obj, key) => {
